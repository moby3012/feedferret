...
- 🔄 **M4 ⭐** — **AI config proposal** (fetch→clean→BYOK→validate→preview→accept) — **M** · Slice 1 shipped (#149): `lib/ai-feed-config.ts` — AI proposes a fulltext/pagefeed config, validated through the real engine before it's trusted. Slice 2 shipped (#155): "✨ Let AI set this up" button in Add feed → From web page (AI-badge candidate card, article-page detection, rate-limited 10/min, notes rendered as plain text). Remaining: **M4-T5** — AI proposal for truncated-feed full-text selectors in feed settings.
