...
- 🔄 **M5** — Optional connectors: **RSSHub** + **changedetection.io** (self-hosted sidecars) — **M ×2** — *M5a (RSSHub) fully shipped 2026-07-20: `lib/rsshub.ts` engine + admin config + the "Add from platform" Add-Feed tab (AI-assisted route proposal, always validated for real before trusting it). M5b (changedetection.io) remains.*
