...
- 🔄 **M4 ⭐** — **AI config proposal** (fetch→clean→BYOK→validate→preview→accept) — **M** · Slice 1 shipped (#149): `lib/ai-feed-config.ts` — AI proposes a fulltext/pagefeed config, validated through the real engine before it's trusted; `runAiPrompt` exported. Remaining: the "✨ Let AI set this up" UX (slice 2) + rate-limit/size guardrails (slice 3).
