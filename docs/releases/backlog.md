...
| **`prefers-contrast: more` Support** | Detect the `prefers-contrast: more` OS-level media query. When active, increase border widths, strengthen separator lines, and boost text contrast beyond the default theme. Implemented as a CSS media query block — no JS required. | S |
| **Accessibility Statement Page** | Public `/accessibility` page listing FeedFerret's current WCAG 2.2 AA conformance status, known issues, and contact information for accessibility feedback. Updated after each release. | S |
