...
| **Website Scraping Feeds** | ✅ Done — the **page→feed builder** (Scout Studio): subscribe to any HTML page via ranked CSS/XPath selectors, engine-validated, also creatable over REST/MCP (`/connectors/page/*`). See [`../scout-studio.md`](../scout-studio.md). | L | — |
| **WebSub / PubSubHubbub** | Subscribe to WebSub hubs declared in feed `<link rel="hub">` tags. Requires a public callback URL. | L | backlog |
| **Offline-First Mutations** | Queue mark-read/star in IndexedDB; sync on reconnect via Service Worker background sync. Overlaps with the Perf & UX audit's optimistic-UI work. | L | backlog |
