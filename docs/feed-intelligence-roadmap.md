...
- [x] **F5** · Export / "Send to" Obsidian · Wallabag · Readwise · Pocket — `S–M` — *shipped 2026-07-20: Obsidian (`obsidian://new` deep link, no API/credentials) and Wallabag (self-hosted, OAuth2 + `/api/entries`), both matching this project's self-hosting ethos. Readwise/Pocket deliberately deferred — their "save article" API contracts couldn't be confidently verified from training knowledge alone; do that verification before adding them rather than guessing.*
- [x] **F6** · Auto-mute + notify on persistently-failing feeds (we already track feed health) — `S–M` — *shipped 2026-07-20: `Feed.consecutiveFailureCount`/`autoMuted` + `User.autoMuteFailingFeedsAfter` (default 10, 0 = disabled). Background sync skips auto-muted feeds; a one-time in-app notification explains why; Health tab gets a mute/unmute toggle (unmute resets the counter and gives one more chance).*
- [x] **F7** · PWA share-target bookmarklet ("Share → FeedFerret" → starts the page→feed flow) — `S–M` — *shipped 2026-07-20: manifest `share_target` + `?sharedUrl=` deep link opens the Add-Feed dialog on "From web page" pre-filled. Manifest validity and the deep-link route are verified; actual OS-level share-sheet invocation from an installed PWA is not (can't be simulated in a sandboxed dev server).*
