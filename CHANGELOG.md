...
### Fixed (2026-07-21 — article/feed timestamps now respect a per-user display timezone)

- Every timestamp in the app (article list, reader, shared-search page) was rendered in whatever timezone the server process happened to default to (UTC in most Docker deployments) instead of the reader's own — e.g. a German user saw article times offset by their UTC+1/+2 difference. Added a **Display timezone** setting (Settings → Reading): "Automatic" auto-detects the browser's IANA zone on first visit (seeded into a `timezone` cookie, `components/timezone-sync.tsx`), or the user can pick any zone explicitly, which always wins. The fix is centralized in next-intl's ambient `timeZone` (`i18n/request.ts`), so it applies everywhere `useFormatter`/`getFormatter` is already used — no per-component changes needed. New `lib/timezone.ts` (`isValidTimeZone`, unit-tested) and `app/actions/timezone.ts`.
...
- A failed RSSHub route used to show a bare `Fetch failed: 503`, hiding RSSHub's actual explanation. FeedFerret now surfaces a snippet of the upstream body with the failure — e.g. `Fetch failed: 503 — Twitter API is not configured` or `— This channel does not exist` — so it's clear the problem is a RSSHub-side route requirement (credentials, wrong identifier, unavailable route), not FeedFerret. Implemented as an opt-in `includeErrorBody` option on the SSRF fetch (ordinary feed syncs keep their terse errors), HTML error pages are tag-stripped and truncated. `docs/self-hosting.md` gained a route-troubleshooting table and a credential-free smoke-test route (`/github/repos/DIYgod/RSSHub`) to confirm the connector wiring independent of any one platform's quirks.

### Fixed (2026-07-20 — optional connectors on the internal Docker network were SSRF-blocked)

- RSSHub and changedetection.io run on the same private Docker network as FeedFerret, so their base URLs are private hostnames (e.g. `http://rsshub:3000/`) — which FeedFerret's SSRF guard blocked as internal addresses unless the operator flipped the instance-wide "Allow internal feed URLs" switch. This made the RSSHub connector unusable out of the box: route validation ("internal URL forbidden") and every ongoing sync of a connector-backed feed were rejected. FeedFerret now automatically trusts the **specific configured connector host(s)** for internal fetches — narrowly, only those hosts (new `lib/connector-hosts.ts`), covering both validation and background sync — without weakening SSRF for arbitrary feeds. RSSHub route validation now uses `allowInternal` for its own base URL (only the route path varies, so the host is always the trusted RSSHub instance), matching how the changedetection.io connector's own API calls already worked. `docs/self-hosting.md`'s SSRF notes were rewritten to explain this, plus a heads-up that env-configured connector fields intentionally read back empty in the admin UI (env takes precedence — the connector is still active).
...
- **F6 — auto-mute + notify on persistently-failing feeds** — `Feed.consecutiveFailureCount` now increments on each sync failure and resets on any success (`lib/feed-auto-mute.ts`, unit-tested in isolation from the DB/notification wiring around it). Once a feed crosses the user's configurable `autoMuteFailingFeedsAfter` threshold (Manage Feeds → Health tab; default 10, 0 disables it entirely), it's flagged `Feed.autoMuted` — background sync (`syncUserFeeds`/`syncAllFeeds`) stops actively re-fetching it (no more wasted cycles hitting a dead URL), and a one-time in-app notification (`type: "feed_auto_muted"`) explains why. A manual "Refresh now" on a muted feed still works and can self-recover it. The Health tab gains a mute/unmute toggle per feed; unmuting resets the failure counter and gives the feed one more chance, per spec.
- **F7 — PWA share-target ("Share → FeedFerret")** — `public/manifest.json` gained a `share_target` block (`action: "/"`, GET, mapping the OS share sheet's `url`/`text` fields onto `?sharedUrl=`/`?sharedText=`). `app/page.tsx`'s existing `?addFeed=1` deep-link handling gained a sibling `?sharedUrl=` handler that opens the Add-Feed dialog directly on the "From web page" tab with the shared URL pre-filled (`components/page-feed-panel.tsx` gained an `initialUrl` prop) — sharing a no-RSS page from any app drops straight into Scout Studio's page→feed builder. Verified: the manifest is valid JSON and parses with a real `share_target`, and the `?sharedUrl=` route serves without error. **Not verified** (can't be simulated in a sandboxed dev server): the actual OS-level "Share → FeedFerret" invocation from an installed PWA's native share sheet.

### Added (2026-07-20 — M5b: changedetection.io connector)

- **"Monitor this page" — a 5th Add-Feed tab for the changedetection.io connector** (`components/changedetection-panel.tsx`, `lib/changedetection.ts`) — only appears once an admin has configured a self-hosted [changedetection.io](https://changedetection.io) instance. Turns *any* page — including JS-rendered ones, since changedetection.io does its own browser rendering — into a feed of its changes over time, via its REST API (`POST /api/v1/watch`). Confirmed directly from changedetection.io's own source (not guessed): creating and reading watches needs **two separate secrets** — the REST API key, and a distinct "RSS access token" required as a `?token=` query param on the per-watch RSS route, which the API key does not satisfy. When an AI provider is configured, reuses M4's existing fulltext-selector-proposal engine (`lib/ai-feed-config.ts`) verbatim to suggest a content selector for the watch — only used once that engine's own real-page validation passes. A freshly created watch's feed has no items until changedetection.io has completed at least two checks on the page (confirmed from its source: the RSS route 400s until then) — the UI sets that expectation explicitly rather than pretending to validate synchronously, unlike the RSSHub connector where a route can be validated immediately. This completes **M5** (both optional connectors) and **M5-T3** — `docs/self-hosting.md` gained a full "Optional Connectors" section (render sidecar, RSSHub, changedetection.io: setup, minimal compose snippets, SSRF/network-isolation notes).

### Fixed (2026-07-20 — mobile search/sidebar)

- On iPhone, submitting a search from within the mobile sidebar left the sidebar Sheet open on top of the revealed search results — since the Sheet is a full-viewport overlay, this also blocked touch/scroll input, making search results look unscrollable. The search form's submit handler now closes the sidebar alongside the search dialog.
